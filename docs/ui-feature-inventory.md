...
|---|---|---|---|
| 1 | Foundation (auth, multi-tenancy/RLS, Settings Registry, admin audit log) | yes | Cross-cutting; §15 Settings (Security); Admin Terminal → Audit log |
| 2 | Catalog & Media | yes | §3 Products |
| 3 | Custom Domain & TLS | yes | §15 Settings → Domains tab |
| 4 | Theme Engine & Storefront Rendering | yes | Buyer-Facing Storefront (Part 4); §8 Design Studio |
| 5 | Discovery & Merchandising | yes | Buyer-Facing Storefront → Search page |
| 6 | Listing Moderation Engine | yes | §3 Products (moderationStatus); Admin Terminal → Moderation queue |
| 7 | Shipping, Tax & Discounts | yes | §9 Shipping & Tracking; §7.4 Discounts tab |
| 8 | Suppliers & Printify Adapter | yes | §10 Suppliers |
| 9 | Orders, Cart & Checkout | yes | §2 Orders; Buyer-Facing Storefront → Cart/Checkout/Order-status/Order-confirmation |
| 10 | Seller Dashboard UI (foundation) | yes | Cross-cutting (this whole document's subject) |
| 11 | Commission & Invoicing Engine | yes, now dormant (0% commission since Module 74) | §14 Billing & Plan (background) |
| 12 | Trust & Safety System | yes | §15 Settings (Identity verification); Admin Terminal → Trust & Safety |
| 13 | Seller Account Security (2FA + devices) | yes | §15 Settings → Security |
| 14 | Plans, Pricing & Business Guard-Rails | yes | §14 Billing & Plan; Admin Terminal → Plans |
| 15 | Customers, Reviews & Data Portability | yes | §5 Customers; §5c Reviews; Buyer-Facing Storefront → Reviews-with-media submission flow |
| 15.5 | Storefront Buyer Purchase Flow & Store Branding | yes | Buyer-Facing Storefront (Part 4); §15 Settings (Store branding) |
| 16 | Seller Onboarding Wizard | yes | §1 Home (Setup checklist) |
| 17 | Admin Control Plane completion | yes | Admin Terminal → Home/Search/Sellers/Messages |
| 18 | External-SaaS Integration Hooks | yes | §7.6–7.7 Marketing hub (Meta feed, Social Media SaaS handoff); Admin Terminal → External API clients |
...
| 29 | Delivery-Time Badges | yes | Buyer-Facing Storefront → Product detail page (DeliveryBadge); §10 Suppliers (data source) |
...
| 35 | Staff Accounts, plan-tier | yes | §13 Staff |
| 36 | Admin Email Section | yes | Admin Terminal → Email |
| 37 | Advanced Granular Admin Control | yes | Admin Terminal → Moderation queue (force remove/restore) |
...
| 42 | Emotional & Retention Layer (milestone celebrations) | **yes — corrected, was stale** | §1 Home — `MilestoneBanner`, real, backed by `GET .../milestones/recent` |
| 43 | Community & Belonging | **not built** | N/A — spec only |
| 44 | No Free Plan — First Month Entry Pricing rework | yes | §14 Billing & Plan (background) |
...
| 48 | Facebook/Instagram Shop Feed + WhatsApp Catalog Links | yes | §7.6 Marketing hub — **corrected, was stale**: fixed in the Phase 4 close-out, two real cards (Meta Catalog feed + WhatsApp product-share link) |
...
| 57 | Invoice/Receipt Customization | yes | §12 Reports (Invoice customization card) |
| 58 | Advanced Store SEO Control | yes | §3b Products form (Advanced SEO card); §15 Settings (store-level Advanced SEO) |
| 59 | Combined Entry-Flow Payment | yes, now superseded/dormant (retired by Module 73) | §14 Billing & Plan (confirmed: does not exist as a separate screen) |
| 60 | Wallet/Commission Re-Verification vs Four-Tier Plans | yes | background only, no UI |
| 61 | Four-Tier Plan Pricing Model | yes | §14 Billing & Plan |
| 62 | Seller Payment Gateway Connect | yes | §11 Payments |
| 63–72 | Subscription Business Readiness (MRR analytics, retention window, renewal reminders, downgrade rule, gateway health, support SLA, health funnel, monthly report, discount-abuse prevention, refund policy) | **corrected, was a stale blanket claim: 8 of 10 backend-complete/no frontend, 2 of 10 (63, 67) now have a frontend consumer** | Module 63 (MRR analytics) surfaces in Admin Terminal → Finance Terminal (`GET /admin/analytics/mrr` consumed for the Revenue overview tiles); Module 67 (gateway health) surfaces in Admin Terminal → System status ("Payment gateway health" card, Phase 6b). The other 8 (64–66, 68–72) are confirmed still frontend-less. Module 69's seller-health-funnel endpoint (`GET admin/analytics/seller-funnel`) specifically re-confirmed to have zero UI consumer anywhere. |
| 89 | Bulk-action backend endpoints (moderation queue + wallet top-up verification) | yes | Admin Terminal → Moderation queue (bulk approve/reject); Wallet top-ups (bulk verify/reject) — both now real `POST .../bulk-decide` endpoints, replacing the old client-side `Promise.all` fan-out |
...
| 91 | Platform Merchant Connection (founder-directed scope addition) | yes | Admin Terminal → Platform merchant connection. UZEYN itself as the connected merchant, Module 62's exact adapter architecture, dormant until an admin connects real credentials and activates. Financial-safety hardened (idempotency, amount-mismatch flagging, timeout fail-safe, weekly reconciliation) in the Phase 8 audit. |
| 73 | Subscription-Only Renewal Mechanism | yes | §14 Billing & Plan |
| 74 | Final Plans + First-Cycle Discount | yes | §14 Billing & Plan |
...
### Corrected (Phase 8, was stale) — Home now renders everything the original brief asked for
This section previously described a bare 2-card dashboard and flagged the 4 gauges/recent-orders/milestone banner as "design target, not built." **That's no longer true** — a later pass (outside the numbered phases, commit `438a18f`) built all of it directly into this file. The doc was never updated to match; corrected here.

### Screen states, in real render order

**State 1 — Onboarding wizard** (while `!onboarding.completedAt`) — unchanged from before, see the `OnboardingWizard` component in this same file: "Setup checklist" card, one row per step (theme/logo/product/domain), `Badge` "Done"/"To do", real per-step actions.

**State 2 — Empty-products gate** (onboarding done, 0 products) — "Add your first product" `EmptyState` card, button → `/products/new`.

**State 3 — Unpublished gate** (products exist, `!store.publishedAt`) — "Ready to go live?" card, button "Publish store" → `POST /stores/:storeId/publish`, real errors surfaced via `Alert` (missing payment method / missing identity verification).

**State 4 — Dashboard (real steady state, now real dashboard content):**
- `MilestoneBanner` — **real, built.** Backed by `GET /stores/:storeId/milestones/recent` → `{ milestone: Milestone | null }`; renders nothing when null (dismissible, not decorative-only).
- **4 `GaugeCard`s** — Sales (30d), Revenue (30d), Repeat customers, Average order value. All computed from real endpoints already fetched client-side: `GET .../analytics/sales-over-time?bucket=day&start=<60d ago>` (summed into current/prior 30-day windows for the period-over-period `percent`, via `periodRatio()`/`periodChangeHint()` in this file) and `GET .../analytics/overview` (`repeatCustomerRate`, `aov`). Each gauge has a real `isEmpty`/`emptyMessage` state when there's no sales history yet — not just a happy-path render.
- **"Recent orders"** card — real 5-most-recent list from the already-fetched `GET .../orders?limit=100`, sorted by `placedAt` desc, `AvatarInitials` + order number/buyer email/amount/status `Badge`, links to the full Orders list. Real `EmptyState` when there are none.
- **"Needs attention"** card — pending-order count + live product count, links to Orders/Products.

### Notes carried over from the original audit, still accurate
- `overview` does **not** return revenue/order-count directly — Sales/Revenue are summed from `sales-over-time`, exactly as this page does it.
- No `customer` relation is included in the orders list query — the recent-orders card uses `buyerEmail`/`AvatarInitials` rather than a joined customer name.
- **Loading:** `PageSpinner` while any of products/orders/onboarding/store/overview/salesOverTime is null.
- **Error:** only the publish action has visible error handling (`Alert`); every other fetch fails silently (`.catch(() => fallback)`).
- **Modals:** none. **Real-time:** none.
- **Motion:** no `Reveal`/GSAP treatment on this page (confirmed via Phase 8c audit) — flagged as the single highest-impact motion gap in the whole platform, since this is the first screen every seller sees and its stat-grid/list shapes exactly match `orders`/`customers`, both of which already have it. Scheduled for the Phase 8 motion-consistency fix.
- **Mobile:** relies entirely on the shared Sidebar drawer + responsive grid breakpoints on the gauge/card grids.
